...
#     with rasterio.open(target_geotiff_file, 'w', 
#                        driver='GTiff', 
...
#       # Rasterize the DataFrame into the raster
#       df['predicted_swe'].fillna(0, inplace=True)  # Fill NaN values with 0 for rasterization
#       arr = np.array(df['predicted_swe'].values, dtype='float32')
      
#       rasterize(
#           [(lon, lat, value) for lon, lat, value in zip(df['lon'], df['lat'], df['predicted_swe'])],
...
#     )
    ...
    # Write the resampled values to the GeoTIFF
